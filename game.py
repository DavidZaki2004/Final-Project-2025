...
if __name__ == "__main__":
    ...
    while True:
        ...
        if mode == "1":
            ...
            o_player = choose_player(2, game_type)
            if game_choice == 1:
                result = run_tictactoe(game, x_player, o_player, print_game=True)
            else:
                result = run_connect4(game, x_player, o_player, print_game=True)
            print(f"\n{game_type} Game Over! Result: {result}")
        elif mode == "2":
            run_experiment()
        play_again = input("Do you want to play another game or experiment? (yes/no): ").strip().lower()
        if play_again != "yes":
